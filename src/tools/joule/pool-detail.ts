...
export async function getPoolDetails(
  ...
  try {
    const allPoolDetailsResponse = await fetch(
      "https://price-api.joule.finance/api/market"
    );

    const allPoolDetails = await allPoolDetailsResponse.json();

    const poolDetail = allPoolDetails.data.find((pool: any) =>
      pool.asset.type.includes(mint)
    );
    ...
    return {
      ...
      totalBorrowed:
        Number(poolDetail.totalBorrowed) / poolDetail.asset.decimals,
