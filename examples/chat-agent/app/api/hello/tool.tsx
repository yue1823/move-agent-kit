...
class TweetNFTTool extends Tool {
	name = "tweet_nft"
	description = `
	...
	async _call(args: string): Promise<string> {
		try {
			const parsedInput = JSON.parse(args)
			const { tag, text, to_address, nft_url } = parsedInput

			if (!tag || !text || !to_address || !nft_url) {
				return "Error: Missing required input parameters (tag, text, to_address, nft_url)."
			}

			// Validate address format (basic check)
			if (!to_address.startsWith("0x") || to_address.length !== 66) {
			...
			const transactionResult = await this.executeAptosTransaction(tag, text, to_address, nft_url)

			if (transactionResult.success) {
				return JSON.stringify({
					data: "Tweet NFT creation initiated successfully.  Check the transaction status on the Aptos blockchain.",
					transactionHash: transactionResult.transactionHash, // Include transaction hash for verification
				})
			} else {
				return JSON.stringify({ error: `Tweet NFT creation failed: ${transactionResult.errorMessage}` })
			}
		} catch (error: unknown) {
	...
	private async executeAptosTransaction(
		tag: string,
		text: string,
		to_address: string,
		nft_url: string
